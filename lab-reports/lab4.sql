...
-- List the course_ids of courses that have more than 2 students enrolled (Use HAVING)
select c.course_id, count(*) as count from courses as c
join enrollment as e
...
-- Find the student_ids of students who are enrolled in exactly 2 courses.
select s.student_id, count(*) from 
students as s
join enrollment as e
on e.student_id = s.student_id
join courses as c
...
-- Write a query to display the name of each student alongside the course_ids they are enrolled in.
select s.name, c.course_id from 
students as s
join enrollment as e
on e.student_id = s.student_id
join courses as c
...
-- Retrieve the names of students and the names of the courses they are enrolled in 
...
-- List all courses and the number of students enrolled in each. Ensure courses with zero students are also included in the result
...
-- Find the names of all students who are actively taking 'Database Management Systems'.
...
-- Identify any students who are not enrolled in any course.
...
-- Find the names of students who are enrolled in the course with the highest credit hours 
select s.name from students as s
join enrollment as e
on s.student_id=e.student_id
join courses as c
on c.course_id = e.course_id
where c.credit_hour in
(
select max(c.credit_hour) from courses as c
...
join courses as c
on c.course_id = e.course_id
where c.credit_hour=4;

-- List the courses that have an enrollment count strictly higher than the average enrollment count across all courses.
select c.name from students as s
join enrollment as e
on s.student_id=e.student_id
join courses as c
on c.course_id = e.course_id
